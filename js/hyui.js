$(function() {
    ...
    /*------------------------------------*/
    //////////字型大小 font-size dropdwon////////
    /*------------------------------------*/
    $('.function_panel .font_size').children('ul').hide();
    $('.function_panel .font_size').prepend('<a href="#" class="fontsizeButton">fontsize按鈕</a>');
    var _fontsizeButton = $('.fontsizeButton');
    _fontsizeButton.off().click(function(e) {
        $(this).siblings('ul').stop(true, true).slideToggle();
        e.preventDefault();
    });
    _fontsizeButton.keyup(function(event) {
        $(this).siblings('ul').stop(true, true).slideDown();
    });
    $('.function_panel .font_size').find('li:last>a').focusout(function(event) {
        $(this).parent().parent('ul').hide();
    });
    // 點外面關閉share
    $(document).on('touchend click', function(e) {
        var container = $(".function_panel .font_size");
        if (!container.is(e.target) && container.has(e.target).length === 0) {
            $('.function_panel .font_size ul').hide();
        }
    });
    /*------------------------------------*/
    /////////////字型大小 font-size//////////
